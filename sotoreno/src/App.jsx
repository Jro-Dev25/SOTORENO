...
// // ✅ NUEVO: Importar React y useState hook para manejo de estado
// import React, { useState } from 'react';

// // ⚪ ORIGINAL: Imports de imágenes y estilos (sin cambios)
// // IMAGES
// import Arrow from "./IMGs/flecha.png";
// import Logo from './IMGs/logo_nobg.png';
// import MainPic from "./IMGs/main pic.png";
...
// const App = () => {
//   // ===================================================================
//   // ✅ NUEVO: DECLARACIÓN DE ESTADOS
//   // ===================================================================
  
//   // Estado para controlar la modalidad de reunión seleccionada (presencial o videoconferencia)
//   const [modalidad, setModalidad] = useState('presencial');
  
//   // Estado para almacenar todos los datos del formulario de reunión
//   const [reunionData, setReunionData] = useState({
//     nombre: '',
//     email: '',
//     telefono: '',
...
//   // Estado para almacenar los datos del formulario de mensajes
//   const [mensajeData, setMensajeData] = useState({
//     nombre: '',
//     email: '',
//     telefono: '',
...
//   // Estado para controlar cuando se está enviando información (loading state)
//   const [loading, setLoading] = useState(false);
...
//   // ===================================================================
//   // ✅ NUEVO: EVENT HANDLERS (Manejadores de eventos)
//   // ===================================================================
  
//   // Función que se ejecuta cada vez que el usuario escribe en el formulario de reunión
//   const handleReunionChange = (e) => {
//     setReunionData({ 
//       ...reunionData,                    // Mantiene todos los valores actuales
//       [e.target.name]: e.target.value    // Actualiza solo el campo que cambió
//     });
//   };

//   // Función que se ejecuta cada vez que el usuario escribe en el formulario de mensaje
//   const handleMensajeChange = (e) => {
//     setMensajeData({ 
//       ...mensajeData,                    // Mantiene todos los valores actuales
//       [e.target.name]: e.target.value    // Actualiza solo el campo que cambió
//     });
//   };
...
//   // ===================================================================
//   // ✅ NUEVO: FUNCIÓN PRINCIPAL - AGENDAR REUNIÓN
//   // ===================================================================
  
//   const agendarReunion = async () => {
//     // PASO 1: Validar que los campos obligatorios estén llenos
//     if (!reunionData.nombre || !reunionData.email || !reunionData.telefono) {
//       setStatusMessage('Por favor completa todos los campos requeridos');
//       setTimeout(() => setStatusMessage(''), 3000); // Ocultar mensaje después de 3 segundos
//       return;
//     }
...
//     // PASO 3: Activar estado de carga (deshabilita botón, muestra "Agendando...")
//     setLoading(true);

//     try {
//       // PASO 4: Preparar datos completos para guardar
//       const data = {
//         ...reunionData,                      // Todos los campos del formulario
//         modalidad,                           // Modalidad seleccionada (presencial/videoconferencia)
//         timestamp: new Date().toISOString(), // Fecha y hora exacta del envío
//         id: `reunion_${Date.now()}`          // ID único para esta reunión
//       };

//       // PASO 5: Guardar datos en almacenamiento persistente
//       localStorage.setItem(data.id, JSON.stringify(data));  // ← Cambiado de window.storage

//       // PASO 6: Preparar contenido del email
//       const emailContent = `
//         NUEVA REUNIÓN AGENDADA
        
//         Modalidad: ${modalidad}
//         Nombre: ${reunionData.nombre}
//         Email: ${reunionData.email}
//         Teléfono: ${reunionData.telefono}
//         Fecha: ${reunionData.fecha}
//         Hora: ${reunionData.hora}
//         Tipo de proyecto: ${reunionData.tipoProyecto}
//         Descripción: ${reunionData.descripcion}
//       `;

//       // PASO 7: Enviar email usando EmailJS API
//       const response = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({
//           service_id: 'service_sotoreno',       // ⚠️ REEMPLAZAR con tu Service ID
//           template_id: 'template_reunion',      // ⚠️ REEMPLAZAR con tu Template ID
//           user_id: 'YOUR_PUBLIC_KEY',           // ⚠️ REEMPLAZAR con tu Public Key
//           template_params: {
//             to_email: 'sotoreno-99@outlook.com',
//             from_name: reunionData.nombre,
//             from_email: reunionData.email,
...
//       // PASO 8: Verificar si el email se envió correctamente
//       if (response.ok) {
//         setStatusMessage('¡Reunión agendada exitosamente! Te contactaremos pronto.');
        
//         // PASO 9: Limpiar formulario después de envío exitoso
//         setReunionData({
//           nombre: '',
//           email: '',
//           telefono: '',
...
//         });
//       } else {
//         setStatusMessage('Reunión guardada. Email pendiente de configuración.');
//       }
//     } catch (error) {
//       // PASO 10: Manejo de errores
//       console.error('Error al agendar reunión:', error);
//       setStatusMessage('Reunión guardada localmente.');
//     }

//     // PASO 11: Desactivar estado de carga
//     setLoading(false);
    
//     // PASO 12: Ocultar mensaje después de 5 segundos
//     setTimeout(() => setStatusMessage(''), 5000);
//   };

//   // ===================================================================
//   // ✅ NUEVO: FUNCIÓN PRINCIPAL - ENVIAR MENSAJE
//   // ===================================================================
...
//     try {
//       // PASO 4: Preparar datos completos
//       const data = {
//         ...mensajeData,
...
//       template_id: process.env.REACT_APP_EMAILJS_TEMPLATE_MENSAJE,
//       user_id: process.env.REACT_APP_EMAILJS_PUBLIC_KEY,
//       template_params: {
//       to_email: process.env.REACT_APP_EMAIL_TO,
//       from_name: mensajeData.nombre,
//       from_email: mensajeData.email,
//       reply_to: mensajeData.email,   // optional
//       message: emailContent
//     }
//   })
// });

//       // PASO 8: Verificar respuesta
//       if (response.ok) {
...
//         });
//       } else {
//         setStatusMessage('Mensaje guardado. Email pendiente de configuración.');
//       }
//     } catch (error) {
//       console.error('Error al enviar mensaje:', error);
//       setStatusMessage('Mensaje guardado localmente.');
//     }
...
//   // ===================================================================
//   // JSX - RENDER DEL COMPONENTE
//   // ===================================================================
  
//   return (
//     <section className='Sotoreno'>
//       {/* ⚪ HEADER - Sin cambios en estructura, solo en contenido */}
//       <header id='header' className='Header-head'>
//         <section className='Header-body'>
//           <div className='header-top'>
//             <div className='header-titulo'>
...
//           <div className="header-main">
//             <div className='header-menu'>
//               <img src={Arrow} alt="Arrow" className="arrow" id="arrow-main"/>
//               <ul>
//                 <li className="hover"><a href="#nosotros">Nosotros</a></li>
//                 <li className="hover"><a href="#servicios">Servicios</a></li>
//                 <li className="hover"><a href="#reuniones">Reuniones</a></li>
...
//             </div>
//             <div className="main-pic">
//               <img src={MainPic} alt="MainPic" id="MainPic" className="img"/>
//             </div>
//           </div>
...
//             <section className="nosotros-section">
//               <h2 id="nosotros">Nosotros</h2> 
//               <p>En Sotoreno creemos que la arquitectura es la
//               oportunidad de <span>transformar ideas</span> en espacios que
//               inspiran, conectan y <span>mejoran la vida</span> de las personas.
//               Somos un taller dedicado al <span>diseño integral</span>, donde
//               cada proyecto se aborda con una mirada estratégica,
//               creativa y profundamente humana creando una
//               mímesis entre <span>función y estética.</span></p>
//             </section>
//           </div>
//         </section>
//       </header>
...
//           <div className="servicios-container">
//             <div className="servicios-header">   
//               <img src={Cabaña} alt="main" className="servicios-main-img" />
//               <img src={Libreria} className="libreria img"/>
//               <div className="servicios-twoimg">
//                 <img src={Casa7} className="casa7 img" />
//                 <img src={Casa1} className="casa1 img" />
//               </div>
//               <img src={Casa6} className="casa6 img" />
//             </div>
...
//                   <li>Remodelación</li>
//                   <div className="servicios-lista-box_arrow">
//                     <img src={Arrow} alt="Arrow" className="arrow arrow_servicios" />
//                   </div>
//                 </ul>
//                 <div>
//                   <img src={Casa8} className="casa8 img"/>
//                   <img src={Casa2} className="casa2 img"/>
//                 </div>
//               </div>
//             </div>
//           </div>
...
//           <div className="servicios-galeria">
//             <p className="servicios-desc">
//             En Sotoreno creemos que la arquitectura es la oportunidad de
//             <span> transformar ideas </span>
//             en espacios que inspiran, conectan y
//             <span> mejoran la vida </span>
//             de las personas. Somos un taller dedicado al
//             <span> diseño integral</span>, donde cada proyecto se aborda con una
//             mirada estratégica, creativa y profundamente humana creando una
//             mímesis entre
//             <span> función y estética.</span>
//             </p>
//           </div>
//         </section>
//       </main>
...
//       {/* ===================================================================
//           ✅ FOOTER - SECCIÓN MODIFICADA CON FUNCIONALIDAD
//           =================================================================== */}
//       <footer>
//         {/* ✅ NUEVO: Mensaje de estado (éxito/error) */}
//         {statusMessage && (
//           <div style={{
//             padding: '15px',
//             margin: '20px auto',
//             maxWidth: '600px',
//             backgroundColor: statusMessage.includes('exitosamente') ? '#d4edda' : '#f8d7da',
//             color: statusMessage.includes('exitosamente') ? '#155724' : '#721c24',
//             borderRadius: '5px',
//             textAlign: 'center'
//           }}>
//             {statusMessage}
...
//           </div>
//           <div className="arrow-reuniones_container">
//             <img src={Arrow} alt="Arrow" className="arrow" id="arrow-reunion" />
//           </div>
//         </section>
        
//         <section className="reuniones-form">
//           {/* ✅ MODIFICADO: Botones ahora son interactivos con estado dinámico */}
//           <div className="reuniones-btn">
//             <button 
//               id="presencial" 
//               className={`btn btn_presencial ${modalidad === 'presencial' ? 'active' : ''}`}
//               onClick={() => setModalidad('presencial')} // ✅ NUEVO: onClick handler
//               style={{
//                 backgroundColor: modalidad === 'presencial' ? 'rgb(6, 116, 178)' : '#fff',
//                 color: modalidad === 'presencial' ? '#fff' : 'rgb(147, 147, 147)'
//               }}
...
//               id="videoconferencia" 
//               className={`btn btn_videoconferencia ${modalidad === 'videoconferencia' ? 'active' : ''}`}
//               onClick={() => setModalidad('videoconferencia')} // ✅ NUEVO: onClick handler
//               style={{
//                 backgroundColor: modalidad === 'videoconferencia' ? 'rgb(6, 116, 178)' : '#fff',
//                 color: modalidad === 'videoconferencia' ? '#fff' : 'rgb(147, 147, 147)'
//               }}
...
//           {/* ✅ MODIFICADO: Inputs ahora son controlados (controlled components) */}
//           <div className="reuniones-inputs">
//             <label htmlFor="">Nombre completo</label>
//             <input 
//               type="text" 
//               name="nombre"                          // ✅ NUEVO: name attribute
//               value={reunionData.nombre}             // ✅ NUEVO: value vinculado al estado
//               onChange={handleReunionChange}         // ✅ NUEVO: onChange handler
//               placeholder='Nombre completo' 
//               className="input reunion-input"
//             />            
//             <label htmlFor="">Email</label>
//             <input 
//               type="email" 
//               name="email"                           // ✅ NUEVO
//               value={reunionData.email}              // ✅ NUEVO
//               onChange={handleReunionChange}         // ✅ NUEVO
//               placeholder='Email' 
//               className="input reunion-input"
//             /> 
...
//               type="tel" 
//               name="telefono"                        // ✅ NUEVO
//               value={reunionData.telefono}           // ✅ NUEVO
//               onChange={handleReunionChange}         // ✅ NUEVO
//               placeholder='+52 (667) 123 456 7' 
//               className="input reunion-input"
//             />
...
//               type="date" 
//               name="fecha"                           // ✅ NUEVO
//               value={reunionData.fecha}              // ✅ NUEVO
//               onChange={handleReunionChange}         // ✅ NUEVO
//               className="reuniones-dates-input"
//             />
//             <label htmlFor="">Hora</label>
//             <input 
//               type="time" 
//               name="hora"                            // ✅ NUEVO
//               value={reunionData.hora}               // ✅ NUEVO
//               onChange={handleReunionChange}         // ✅ NUEVO
//               className="reuniones-dates-input"
//             />
//           </div>
...
//           <div className="proyectos">
//             <div>
//               <label htmlFor="tipodeproyectos">Tipo de proyecto</label>
//               <select 
//                 id="tipodeproyectos" 
//                 name="tipoProyecto"                  // ✅ NUEVO
//                 value={reunionData.tipoProyecto}     // ✅ NUEVO
//                 onChange={handleReunionChange}       // ✅ NUEVO
//                 className="tipodeproyecto"
//               >
//                 <option value="">Selecciona un tipo</option>
//                 <option value="residencial">Residencial</option>
//                 <option value="interior">Interior</option>
...
//             <div>
//               <label htmlFor="">Cuentanos sobre tu proyecto</label>
//               <br />
//               <textarea 
//                 name="descripcion"                   // ✅ NUEVO
//                 value={reunionData.descripcion}      // ✅ NUEVO
//                 onChange={handleReunionChange}       // ✅ NUEVO
//                 placeholder='Tienda de artículos deportivos en remodelación interior, exterior y equipamiento.'
//               />
//             </div>
...
//           {/* ✅ MODIFICADO: Botón ahora ejecuta función y muestra estado */}
//           <button 
//             className="btn agendar_btn" 
//             onClick={agendarReunion}                 // ✅ NUEVO: onClick ejecuta función
//             disabled={loading}                       // ✅ NUEVO: Se deshabilita mientras carga
//             style={{
//               backgroundColor: 'rgb(6, 116, 178)',
//               color: 'white',
//               opacity: loading ? 0.6 : 1,           // ✅ NUEVO: Opacidad cuando está cargando
//               cursor: loading ? 'not-allowed' : 'pointer'
//             }}
//           >
//             {loading ? 'Agendando...' : 'Agendar'}  {/* ✅ NUEVO: Texto dinámico */}
//           </button>
//         </section>

//         <hr />
        
//         {/* ✅ MODIFICADO: Formulario de mensajes ahora funcional */}
//         <section className='mensaje-section'>
//           <h2 id="mensaje">Envianos un mensaje</h2>
//           <label htmlFor="">Nombre completo</label>
//           <input 
//             type="text" 
//             name="nombre"                            // ✅ NUEVO
//             value={mensajeData.nombre}               // ✅ NUEVO
//             onChange={handleMensajeChange}           // ✅ NUEVO
//             placeholder='Nombre completo' 
//             className="reuniones-input"
//           />  
//           <label htmlFor="">Email</label>
//           <input 
//             type="email" 
//             name="email"                             // ✅ NUEVO
//             value={mensajeData.email}                // ✅ NUEVO
//             onChange={handleMensajeChange}           // ✅ NUEVO
//             placeholder='Email' 
//             className="reuniones-input"
//           />
...
//             type="tel" 
//             name="telefono"                          // ✅ NUEVO
//             value={mensajeData.telefono}             // ✅ NUEVO
//             onChange={handleMensajeChange}           // ✅ NUEVO
//             placeholder='+52 (667) 123 456 7' 
//             className="reuniones-input"
//           />
...
//           <textarea 
//             name="mensaje"                           // ✅ NUEVO
//             value={mensajeData.mensaje}              // ✅ NUEVO
//             onChange={handleMensajeChange}           // ✅ NUEVO
//             placeholder="¿Trabajarían en un proyecto en Mazatlán?"
//           />
//           <button 
//             className="btn enviar_btn" 
//             onClick={enviarMensaje}                  // ✅ NUEVO
...
//         <div className="logosFooter">
//           <img src={LogoTel} alt="logotel" className='logoFooter'/>
//           <img src={WhatsAppLogo} alt="whatsapp" className='logoFooter'/>
//         </div>
//       </footer>
//     </section>
//   );
